class Product {
  ...
  Product({
    ...
    required this.categories,  // Updated to accept List<String>
  });
...
// Sample list of products with multiple categories
List<Product> foodItems = [
  ...
  Product(
    ...
    rating: 4.5,
    isFavorite: false,
    categories: ['Meat', 'Vegetables', 'Fruits'],  // Multiple categories
  ),
  Product(
    ...
    rating: 4.7,
    isFavorite: false,
    categories: ['Meat', 'Sweets'],  // Multiple categories
  ),
  Product(
...
List<Product> filterProductsByCategory(String category) {
  return foodItems.where((product) => product.categories.contains(category)).toList();
}